...
case "$COMPRESS_FORMAT" in
    "xz")
        ARCHIVE_NAME="${JOB_NAME}_$(date '+%Y%m%d_%H%M%S').tar.xz"
        TAR_OPTIONS="-cJf"
        ;;
    "gz"|"gzip")
        ARCHIVE_NAME="${JOB_NAME}_$(date '+%Y%m%d_%H%M%S').tar.gz"
        TAR_OPTIONS="-czf"
        ;;
    "bz2"|"bzip2")
        ARCHIVE_NAME="${JOB_NAME}_$(date '+%Y%m%d_%H%M%S').tar.bz2"
        TAR_OPTIONS="-cjf"
        ;;
    "zip")
        ARCHIVE_NAME="${JOB_NAME}_$(date '+%Y%m%d_%H%M%S').zip"
        ;;
    *)
...
if [ "$COMPRESS_SUCCESS" = "true" ]; then
    ...
    # 更新环境变量，让备份脚本使用压缩后的文件
    export BACKUP_SOURCE_PATH="$COMPRESS_DIR"
    log_hook_message "已更新备份源路径为: $COMPRESS_DIR"
    
    # 创建清理脚本供 post-backup hook 使用
    cat > "$COMPRESS_DIR/cleanup.sh" << EOF
#!/bin/bash
log_hook_message() {
    echo "[\$(date '+%Y-%m-%d %H:%M:%S')] [CLEANUP] \$1" | tee -a "$LOG_FILE"
}
log_hook_message "清理临时压缩文件: $ARCHIVE_PATH"
rm -f "$ARCHIVE_PATH"
rmdir "$COMPRESS_DIR" 2>/dev/null || true
log_hook_message "清理完成"
EOF
